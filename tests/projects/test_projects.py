...
def test_cannot_access_other_user_project(client: TestClient):
    ...
    token2 = client.post(
        "/auth/login", json={"email": email2, "password": "password123"}
    ).json()["access_token"]
    ...
    )  # Or 403, but service usually raises ValueError("Project not found")
...
def test_update_project_goal(client: TestClient):
    ...
    create_resp = client.post(
        "/projects/",
        json={"name": "Goal Project", "weekly_goal_hours": 5},
        headers=headers,
    )
    p_id = create_resp.json()["id"]

    update_resp = client.patch(
        f"/projects/{p_id}", json={"weekly_goal_hours": 20}, headers=headers
    )
